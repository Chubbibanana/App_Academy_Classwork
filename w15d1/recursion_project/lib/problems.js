...
function flatten(data) {
    // check if data is array, if not, return
    let arr = [];
    for (let i = 0; i < data.length; i++) {
        if (Array.isArray(data[i])) {
            // arr.push(...flatten(data[i]));
...
function fileFinder(directories, targetFile) {
    for (let key in directories) {
      if (
        key === targetFile ||
        fileFinder(directories[key], targetFile) === true
      ) {
        return true;
      }
    }

    // if iterated through the whole thing and no result, return false
    return false;
}
